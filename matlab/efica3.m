function [Wefica, Wsymm, CC, ISRef, ISRsymm, status, icasig]=efica3(X, ini, SaddleTest, leave_mean)
...
if ~exist('leave_mean', 'var')
    leave_mean = false;
end
if ~exist('include_whitening', 'var')
    include_whitening = false;
end

%Default values of parameters
if ~exist('SaddleTest', 'var')
    SaddleTest=true;
end
if ~exist('ini', 'var') || isempty(ini)
    ini= .01*randn(dim);
    %ini = randn(dim, prec);
    %ini_norm = sqrt(sum(ini .* ini));
    %ini = bsxfun(@rdivide, ini, ini_norm);
end

g='rat1';
epsilon=10e-6 * log(dim); %Stop criterion
fineepsilon=10e-7 * log(dim); %Stop criterion for post-estimation
repeat=1;
MaxIt=200*log(dim); %% Maximum number of FastICA iterations
MaxItAfterSaddleTest=20*log(dim); %%Maximum number of iterations after a saddle point was indicated
FinetuneMaxIt=10*log(dim); %%Maximum number of improving iterations
%SupergaussianNL='rat3';  %Nonlinearity used for supergaussian signals 'gaus'-'ggda'-'npnl'-'rat3'
SupergaussianNL='ggda';  %Nonlinearity used for supergaussian signals 'gaus'-'ggda'-'npnl'-'rat3'
...
if ~leave_mean
    Xmean = mean(X,2);
    X = X - Xmean*ones(1,N, prec);    
else
    Xmean = zeros(size(X, 1), 1, prec);    
end
...
mu=zeros(1,dim, prec);nu=zeros(1,dim, prec);beta=zeros(1,dim, prec);
W=ini;
W_before_decor=W;
W = symdecor(W);
NumIt=0;
TotalIt=0;
crit=zeros(1,dim, prec);
while repeat
 while (1-min(crit)>epsilon && NumIt<MaxIt)
   ...
   switch g
    case 'tanh'
     hypTan = tanh(Z'*W);
     W=Z*hypTan/N-ones(dim,1, prec)*sum(1-hypTan.^2).*W/N;
    case 'pow3'
     W=(Z*((Z'*W).^ 3))/N-3*W;
    case 'rat1'
     U=Z'*W;
    ...
    case 'gaus'
     ...
     W=Z*gauss/N-ones(dim,1, prec)*sum(dGauss).*W/N;
   end
  TotalIt=TotalIt+dim;
...
for k=1:dim
    ...
    WW=W*diag(ccc);
    temp = max(sum(WW.^2))<1e-7;
    temp(temp == 0) = 1;
    sloupce=setdiff(1:dim,find(sum(WW.^2)/temp)); % removing almost zero rows
    M=WW(:,sloupce);
    M=symdecor(M);
    if sum(sloupce==k)==1
    ...
    temp = (J.^2*B(k)+J(k)^2*(B+J.^2));
    temp(temp == 0) = 1;
    Werr(k,:)=B(k)*(B+J.^2)./temp;
end
Werr=Werr-diag(diag(Werr));
...
Wefica=Wefica'*CC;
Wsymm=Wsymm'*CC;
icasig=Wefica*X + (Wefica*Xmean)*ones(1,N, prec);
...
function [g,gprime]=nonln(X, prec)

[d N]=size(X);
g=zeros(d,N, prec);
gprime=zeros(d,N, prec);
h=1.06*N^(-1/5);
for i=1:d
    for k=1:N
        Z=(X(i,k)-X(i,:))/h;
...
[V D]=eig(M'*M);
%W=real(M*(V.*(ones(size(M,2),1, prec)*(1./sqrt(diag(D)'))))*V');
%VG = V .* (ones(size(M,2),1, prec)*(1./sqrt(diag(D)')));
temp = sqrt(diag(D)');
temp(temp == 0) = 1;
VG = bsxfun(@times, V, (1./temp));
W=real(M*(VG)*V');