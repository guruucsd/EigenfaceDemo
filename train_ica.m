function varargout = train_ica(varargin)
% TRAIN Application M-file for train.fig
%    FIG = TRAIN launch train GUI.
%    TRAIN('callback_name', ...) invoke the named callback.
% Last Modified by GUIDE v2.5 08-Aug-2012 22:27:49
% Added ICA process 08-Aug-2012 Tomoki Tsuchida
...
function varargout = btnLoadPattern_Callback(h, eventdata, handles, varargin)
    ...
    global Vals Vecs Psii projected patterns Width Height W ica_patterns;
...
function varargout = btnTrain_Callback(h, eventdata, handles, varargin)
    global Vals net ica_patterns W targets Vecs Psii Width Height imgIdx;
disp('train')
    % Disable this button
    ...
    net = nnTrain(net, ica_patterns, targets, nValidationData, nTestData, nMaxEpochs, dBestRMSE);
    ...
    net = nnFeedForward(net, ica_patterns(imgIdx,:));
    ...
    clear Vals net W ica_patterns targets Vecs Psii Width Height imgIdx;
...
function sliderImage_Callback(hObject, eventdata, handles)
    global Vals net W ica_patterns Width Height Vecs Psii imgIdx;

    % get the class label
    imgIdx = get(handles.sliderImage, 'Value');
    imgIdx = floor(imgIdx);
    net = nnFeedForward(net, ica_patterns(imgIdx,:));
    d = size(ica_patterns,2);
    ...
    clear Vals net W ica_patterns Width Height Vecs Psii imgIdx;
